...
def get_player_chunk(player_moves: list[int], list_of_final_transitions: list[int]):
    res: list = []
    ...
    if len(player_moves) > 2:
        for chunk in move_chunks:
            res.append([x for x in get_player_chunk(player_moves[1:], chunk)])
